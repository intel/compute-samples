...
const std::vector<compute::usm_type> migrate_mem_memory_types = {
    compute::usm_type::shared, compute::usm_type::unknown};

class
    clEnqueueMigrateMemINTELWithAllocationCountRegionCountOffsetsAndMemoryTypes
...
HWTEST_P(
  ...
  queue.enqueue_migrate_mem(destination_->get() + destination_offset_,
                            region_count_ * sizeof(uint8_t), 0);
  queue.enqueue_migrate_mem(source_->get() + source_offset_,
                            region_count_ * sizeof(uint8_t), 0);
...
const std::vector<CopyKernelCombination> migrate_mem_arg_offsets = {
    {256, 256, 0, 0}, {256, 128, 128, 0}, {256, 128, 0, 128}};

INSTANTIATE_TEST_SUITE_P(
    Offsets,
    clEnqueueMigrateMemINTELWithAllocationCountRegionCountOffsetsAndMemoryTypes,
    ::testing::Combine(::testing::ValuesIn(migrate_mem_arg_offsets),
                       ::testing::ValuesIn(migrate_mem_memory_types),
                       ::testing::ValuesIn(migrate_mem_memory_types)),
    copy_kernel_test_name_suffix);

HWTEST(clGetEventInfo,
       GivenEventReturnedByMigrateMemThenCommandMigrateMemIsReturned) {
  ...
  compute::command_queue_intel queue(compute::system::default_queue());
  compute::event event =
      queue.enqueue_migrate_mem(source->get(), count * sizeof(uint8_t), 0);
  queue.finish();

  EXPECT_EQ(CL_COMMAND_MIGRATEMEM_INTEL, event.get_command_type());
